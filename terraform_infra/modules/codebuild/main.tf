...
resource "aws_codebuild_project" "my_app" {
  ...
  description   = "Codebuild project for deploying ${var.app_name} application in ${var.env} environment"
  build_timeout = "120"
  ...
  environment {
    ...
    privileged_mode = true #Whether to enable running the Docker daemon inside a Docker container.
